...
public class Eat extends Simulation {
    public Eat(Island island, Settings settings) {
        super(island, settings);
    }

    /**
     * Запускаем обед. Перерисовываем карты
     */
    @Override
    public void run() {
        ...
        for (int y = 0; y < rows; y++)
            executorService.execute(new EatService(getIsland(), y, getSettings()));
